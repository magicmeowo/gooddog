...
### Step 3 · 外观偏好排序（v2，只加分不影响合不合适）
读 4 道 type=pref 外观题（体型 / 毛长 / 毛色 / 长相风格），命中狗的 `appearance` 标签则加分：
- 体型 `size` 命中 +5；`coat_length` 命中 +3；`coat_color`（在 coat_colors 内）+3；`look_style`（在 look_style 内）+4
- 「都行 / 没偏好」不加分。
- **关键：加分用于排序的 `rank` 值不封顶**（显示分仍封顶 100）。否则现实分都顶到 100 的用户（大房子/老手/不差钱），满分群里无法再按外观喜好排序。
- 外观只重排"已经养得了"的狗，不会让养不了的狗因为好看冒头（符合"外观只用来排序"的决策）。
